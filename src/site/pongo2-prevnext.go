...
func (node *tagPrevnextNode) Execute(ctx *pongo2.ExecutionContext, writer pongo2.TemplateWriter) *pongo2.Error {
	...
	if page > 1 {
		...
		if config := context.Public["config"].(*types.Config); config.General.CanonicalNoPagination != nil && *config.General.CanonicalNoPagination {
			includePagination = false
		}
		_, err := writer.WriteString(fmt.Sprintf(`<link rel="prev" href="%s">`, getAlternate(context.Public, langId, page-1, includePagination)))
		if err != nil {
	...
	if page < pages {
		...
		_, err := writer.WriteString(fmt.Sprintf(`<link rel="next" href="%s">`, getAlternate(context.Public, langId, page+1, includePagination)))
		if err != nil {
			return &pongo2.Error{Sender: "tag:prevnext", OrigError: err}
		}
	}
