...
func RebuildSCSS(path string, config *types.Config) error {
	...
	for _, entryBase := range config.Scss.Entries {
		...
		err := func() error {
			...
			if err != nil {
				return fmt.Errorf("can't open scss entry file %s: %s", entry, err.Error())
			}
			defer reader.Close()
			if err := os.MkdirAll(outDir, 0755); err != nil {
				return fmt.Errorf("can't create out directory %s for compiled css: %s",
					outDir, err.Error())
			}
			writer, err := os.OpenFile(outFile, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
			if err != nil {
				return fmt.Errorf("can't create file for compiled css output: %s", err.Error())
			}
			defer writer.Close()
			...
			if err != nil {
				return fmt.Errorf("error compiling scss %s: %s", entry, err.Error())
			}
